...
public class BufferParser {
	...
	public static class BufferParserFactory {
		...
		public BufferParserFactory month(BiConsumer<ParserState, String> consumer) {
			addStage(new MatchMonth(consumer,2));
			return this;
		}
		
		public BufferParserFactory matchDateTime(BiConsumer<ParserState, Integer> consumer) {
			addStage(new MatchDayTime(consumer,1));
			return this;
		}
		
		public BufferParserFactory matchYear(BiConsumer<ParserState, Integer> consumer) {
			addStage(new MatchDayTime(consumer,3));
			return this;
		}
		...
		public BufferParserFactory stringUntilSpecified(BiConsumer<ParserState,String> consumer) {
			addStage(new MatchStringUntil(consumer, ":"));
			return this;
		}
	 ...
	 */
	public static class MatchMonth extends AbstractParserStage<String> {
		int endPoint;

		final static String[] MONTHS = { "Jan", "jan", "Feb", "feb", "Mar", "mar", "Apr", "apr", "May", "may", "Jun",
				"jun", "Jul", "jul", "Aug", "aug", "Sep", "sep", "Oct", "oct", "Nov", "nov", "Dec", "dec" };

		MatchMonth(BiConsumer<ParserState, String> consumer, Integer end) {
			super(consumer);
			endPoint = end;
		}
		...
		public AcceptResult acceptChar(ParserState state, char c) {
			String value;
			if (state.index < endPoint) {
				state.index++;
				accumulate(state, c);
				return AcceptResult.CONTINUE;
			} else {
				// If any characters were accumulated, complete
				if (accumulatedSize(state) > 0 && state.index == endPoint) {
					value = (accumulatedValue(state)).trim();
					boolean isInArray = Arrays.asList(MONTHS).contains(value);
					if (isInArray)
						return AcceptResult.COMPLETE_WITHOUT_CONSUMING;
					return AcceptResult.CANCEL;
				} else {
	 ...
	 * Match a string terminated by a character in a list of end tokens.
	 */
	public static abstract class MatchIntUntil<R> extends AbstractParserStage<R> {

		private final int m_end;

		MatchIntUntil(BiConsumer<ParserState,R> consumer, int end) {
			super(consumer);
			m_end = end;
		}
		...
		@Override
		public AcceptResult acceptChar(ParserState state, char c) {
			if (state.index < m_end &&Character.isDigit(c)) {
				state.index++;
				accumulate(state, c);
				return AcceptResult.CONTINUE;
			} else {
				if (accumulatedSize(state) > 0
						&& ((String.valueOf(c).trim().isEmpty() && state.index == (m_end - 1)))) {
					return AcceptResult.COMPLETE_WITHOUT_CONSUMING;
				}
				// If any characters were accumulated, complete
				if (accumulatedSize(state) > 0 && state.index == m_end) {
					return AcceptResult.COMPLETE_WITHOUT_CONSUMING;
				} else {
	...
	public static class MatchDayTime extends MatchIntUntil<Integer> {
		 MatchDayTime(BiConsumer<ParserState,Integer> consumer, int end) {
			super(consumer, end);
		}
		...
		public Integer getValue(ParserState state) {
			// Trim the leading zeros from this value
		return Integer.parseInt(accumulatedValue(state).trim());
		}
	}
	
	public static class MatchTimeZone extends AbstractParserStage<String> {
		int endPoint;

		final static String[] OFFSETS = { "UTC", "utc", "IST", "ist", "CDT", "cdt", "CST", "cst" };

		MatchTimeZone(BiConsumer<ParserState, String> consumer, Integer end) {
			super(consumer);
			endPoint = end;
		}
		...
		public AcceptResult acceptChar(ParserState state, char c) {
			String value;
			if ((state.index < endPoint)&&Character.isAlphabetic(c)) {
				state.index++;
				accumulate(state, c);
				return AcceptResult.CONTINUE;
			} else {
				...
				if (accumulatedSize(state) > 0 && state.index == endPoint) {
						value = (accumulatedValue(state)).trim();
						boolean isInArray = Arrays.asList(OFFSETS).contains(value);
						if (isInArray)
							return AcceptResult.COMPLETE_WITHOUT_CONSUMING;
						return AcceptResult.CANCEL;
				} else {
